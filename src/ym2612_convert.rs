...
const YM2612_CONVERT_REPO: &str = "ulalume/ym2612_format";
pub(crate) const YM2612_CONVERT_NAME: &str = "ym2612_convert";
...
pub(crate) async fn resolve_ym2612_convert_path(
    ...
    if let Ok(path) = env::var("YM2612_CONVERT_PATH") {
        if !path.is_empty() {
            if let Some(existing) = resolve_existing_command(&path) {
                return Ok(existing);
            }
    ...
    if let Some(path) = which_in_path(YM2612_CONVERT_NAME) {
        return Ok(path);
    }

    match download_ym2612_convert().await {
        Ok(Some(path)) => Ok(path.to_string_lossy().to_string()),
        Ok(None) => Ok(YM2612_CONVERT_NAME.to_string()),
        Err(err) => Err(err),
    }
}

pub(crate) async fn run_ym2612_convert<F>(
    cmd_path: &str,
    context: &str,
    configure: F,
...
fn cached_bin_name() -> String {
    if env::consts::OS == "windows" {
        format!("{YM2612_CONVERT_NAME}.exe")
    } else {
        YM2612_CONVERT_NAME.to_string()
    }
}
...
fn find_cached_binary() -> Option<PathBuf> {
    ...
    for entry in entries.flatten() {
        ...
        if candidate.is_file() {
            ...
            let alt = path.join(YM2612_CONVERT_NAME);
            if alt.is_file() {
...
async fn download_ym2612_convert() -> std::result::Result<Option<PathBuf>, String> {
    ...
    let base = cache_base_dir();
    let version_dir = base.join(format!("{YM2612_CONVERT_NAME}-{}", release.tag_name));
    let bin_name = cached_bin_name();
    ...
    if !bin_path.is_file() {
        ...
        return Err(format!(
            "ym2612_convert binary not found after extracting {asset_name}"
        ));
    }
